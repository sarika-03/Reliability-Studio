  ...
  useEffect(() => {
    ...
    const fetchCorrelationData = async () => {
      ...
      try {
        ...
        const incidentData = await backendAPI.incidents.get(selectedIncident.id);
        ...
        // Fetch metrics, logs, and correlations for the selected incident
        // Note: Metrics and logs are fetched via service name, traces endpoint may not exist yet
        const serviceName = incidentData.service_id || incidentData.service || '';
        const [metrics, logs, correlations] = await Promise.all([
          // Try to get metrics for the service
          backendAPI.metrics.getErrorRate(serviceName).catch(() => null),
          // Try to get logs for the service
          backendAPI.logs.getErrors(serviceName).catch(() => []),
          // Fetch correlation analysis
          backendAPI.incidents.getCorrelations(selectedIncident.id).catch(() => []),
        ]);

        const traces: any[] = []; // Traces endpoint not yet implemented

        // Check if we have any data at all
        const metricsArray = metrics ? (Array.isArray(metrics) ? metrics : [metrics]) : [];
        const logsArray = Array.isArray(logs) ? logs : [];
        const correlationsArray = Array.isArray(correlations) ? correlations : [];
        
        const hasData = metricsArray.length > 0 || logsArray.length > 0 || traces.length > 0;
