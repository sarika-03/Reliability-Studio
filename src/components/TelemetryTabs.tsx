        ...
        try {
            ...
            switch (activeTab) {
                ...
                case 'logs':
                    ...
                    break;
                case 'traces':
                    // Mock or actual traces if available
                    result = [];
                    break;
                case 'k8s':
    ...
    return (
        <div className={styles.container}>
            ...
            <div className={styles.content}>
                {loading ? (
                    <div style={{ color: theme.textMuted }}>Loading {activeTab}...</div>
                ) : (
                    <div>
                        ...
                        {activeTab === 'logs' && Array.isArray(data) && (
                            <div className={styles.dataCard} style={{ padding: 0 }}>
                                {data.length === 0 ? (
                                    <div style={{ padding: '20px', color: theme.textMuted }}>No error logs found.</div>
                                ) : (
                                    data.map((log, i) => (
                        ...
                        {activeTab === 'k8s' && Array.isArray(data) && (
                            <div className={styles.dataCard}>
                                <h4 style={{ margin: '0 0 10px 0', fontSize: '14px' }}>System Resources</h4>
                                {data.map((pod, i) => (
                        ...
                        {activeTab === 'traces' && (
                            <div style={{ color: theme.textMuted, textAlign: 'center', padding: '40px 0' }}>
                                Distributed tracing investigation is coming soon.
                            </div>
                        )}
                    </div>
                )}
